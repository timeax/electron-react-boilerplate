...
const root = path.join(__dirname, '../src/renderer/styles/index.css');

const sassFile = path.join(__dirname, '../src/renderer/styles/utils/_colors.scss');
const Figma = path.join(__dirname, 'figma.json');

const watcher = chokidar.watch(path.join(__dirname, './scripts/scss'), {
    persistent: true,
});
...
function toFigma(list) {
    ...
    for (const group in list) {
        if (Object.hasOwnProperty.call(list, group)) {
            ...
            colors[group.charAt(0).toUpperCase() + group.slice(1)] =
                list[group];
        }
    }
